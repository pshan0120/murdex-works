...
- `gpt_model`/`sovits_model`은 GPT-SoVITS WebUI에서 현재 선택 중인 모델 경로를 그대로 적으면 됩니다 (WebUI 화면의 드롭다운에 표시된 전체 경로 문자열).
- `ref_audio`는 voices.json이 있는 폴더(`audios/voice/`) 기준 상대경로입니다. 3~10초 이내 클립이어야 합니다(GPT-SoVITS 제약).
...
3. 대사는 큰따옴표 안 텍스트만 추출하고, 나머지 HTML 태그는 모두 제거합니다.
